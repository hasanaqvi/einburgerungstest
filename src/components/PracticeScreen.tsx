...
export function PracticeScreen({
  ...
  function optionClass(i: number): string {
    ...
    if (!answered) {
      return `${base} border-gray-200 bg-white hover:border-blue-400 hover:bg-blue-50 active:bg-blue-100 cursor-pointer`;
    }
    if (i === question.ans) return `${base} border-green-500 bg-green-50`;
    if (i === selected) return `${base} border-red-500 bg-red-50`;
    return `${base} border-gray-100 bg-white`;
  }

  function optionTextClass(i: number): string {
    if (!answered) return 'text-gray-800';
    if (i === question.ans) return 'text-green-800 font-medium';
    if (i === selected) return 'text-red-700';
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="flex-1 px-4 py-5 max-w-2xl mx-auto w-full">
        ...
        {/* Question text */}
        <div className="mb-5">
          <p className="text-[18px] font-semibold text-gray-900 leading-snug">{question.de}</p>
          {showQTranslation && question.en && (
            <p className="text-sm text-gray-400 italic mt-2">{question.en}</p>
          )}
          ...
          {question.opts.map((opt, i) => (
            ...
            >
              ...
              {showATranslation && opt.en && (
                <span className="block text-sm text-gray-400 mt-0.5">{opt.en}</span>
              )}
            </button>
          ))}
        </div>
